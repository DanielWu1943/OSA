%% baseline of optimal splitting and FIFO scheduling
...
%% 启动
    ...
    while t <= T
        ...
        %% FIFO调度任务
        selected_task = task_pool(1);
        training_epoch = task_data.Trained_Epochs(selected_task);
        % 更新数据
        t_tr = task_data_pt(selected_task).Train_Time;
        [~, t_opt] = branch_and_bound(num_edges, B(selected_task), t_tr(training_epoch,:), r(1, :), o(selected_task));
    ...
    %% 保存结果到.mat文件
    file_name = sprintf('./plot_taskrelease/baseline_OSFS_results_gamma_%.2f_edges_%d_5000.mat', gamma, num_edges);
